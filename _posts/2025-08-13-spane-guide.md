...
You can run the same evaluation we ran before by changing the anonymizer in the config with a flag:

```bash
python spane/run.py spane/config/config.yaml --pipeline_config spane/spkanon_models/asrbn/config.yaml
```
